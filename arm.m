...
roi_x = 470; % x-coordinate of the top-left corner
roi_y = 138; % y-coordinate of the top-left corner
roi_width = 100;
roi_height = 70;
...
title('Windowed Phase-Shifted Signal');
xlabel('Normalized Freq (Hz)');
ylabel('Amplitude');
grid on;
...
% further dev
% remove buffer (after 0.45Hz, remove y signal due to artefact from
% development sensor emptying its buffer) and copy from 0-0.32 Hz to the
% right side of the signal (since the signal is periodic and mirrored at
% the center of the peak) and use this signal for power spectrum. This is
% beacuse fft has been applied to the signal (fft decomposes to series of
% sin and cos)